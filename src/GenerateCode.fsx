...
// TODO incorporate methods()... see Rust.ml
// TODO testing
// TODO extend code generator to incorporate more of the filtered schemas
...
module CodeGenLL = 
    ...
    let genCpp(schema: Schema) : string[] = 
        ...
        [|
            ...
            match r with
            | RT.SingleScalar -> 
                yield!  simpleFunc(first, manyLines,Some("CATCH_SCALAR"))  
            | RT.SingleTensor -> 
                yield! simpleFunc(first, manyLines,Some("CATCH_TENSOR"))
            | RT.TensorTuple c-> 
                yield!
                    [|
...
let codeGen(metadataPath: string) =
    let schemas = 
        (Parser.schemas(metadataPath),filterFuncs) 
    ...
    ([|yield! CSharpHeader; yield! CSharpBody|],
     [|yield! CPPHeader; yield! schemas |> Array.map CodeGenLL.genCpp |> concatWithNewLine|],
...
let xs,ys,zs = codeGen(path)
File.WriteAllLines(CSharpTarget, xs)
File.WriteAllLines(CPPTarget, ys)
File.WriteAllLines(HTarget, zs)

//let singleLine = "fobar"
//if singleLine.Length < TargetLineWidth then
//    [|sprintf "CATCH_SCALAR(%s%s))" first singleLine|]
//else
//    [| yield sprintf "    %s" first; yield! manyLines |> addFinalSemiColon |] 
//    |> macro("CATCH_SCALAR",true)

//let simpleFunc(firstLine,args) = 
//    [|yield firstLine; yield! args |> multiLineParams |> indent|]
//    |> compressLines 3
//

//simpleFunc("foo(", [|"bar";"fiz";"qak"|]) |> Cpp.macro("FOO",true) |> compressLines 3 |> String.concat "\n"
